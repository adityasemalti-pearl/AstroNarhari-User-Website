...
const VideoCall = () => {
  ...
  const startVideoCall = async () => {
    try {
      ...
      console.log("Starting video call:", bookingId);
       ...
       * initiateVideoCall()
       *
       * Backend checks req.user.role and generates UID 1
       */

      const response = await initiateVideoCall(bookingId);

      console.log("INITIATE VIDEO CALL RESPONSE:", response);
  ...
  const handleEndCall = async () => {
    ...
    try {
      ...
      const response = await terminateVideoCall(
        bookingId,
        actualDuration
      );

      console.log(
        "TERMINATE VIDEO CALL RESPONSE:",
        response
      );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#08050d] px-4">
        <div className="w-full max-w-md rounded-3xl border border-red-500/20 bg-white/5 p-8 text-center text-white backdrop-blur-xl">
          ...
          <button
            onClick={() => navigate(-1)}
            className="mt-6 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 px-8 py-3 text-sm font-bold text-white"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[999] overflow-hidden bg-[#08050d] text-white">
      ...
      <div className="absolute inset-0">
        ...
        >
          {!joined ||
          Object.keys(remoteUsersRef.current)
            .length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center">
              ...
              <p className="mt-2 text-sm text-gray-400">
                Please wait while the astrologer joins
                the consultation.
              </p>
