...
export default function AstrologerModal({ isOpen, onClose, astrologer, onConnect }) {
  if (!isOpen || !astrologer) return null;

  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} transition={{ duration: 0.35, type: "spring", damping: 28 }} className="bg-white rounded-[32px] shadow-2xl max-w-lg w-full overflow-hidden border border-purple-100/80 relative">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 z-20 w-9 h-9 rounded-full bg-black/20 hover:bg-black/40 backdrop-blur-md flex items-center justify-center text-white transition-all cursor-pointer shadow-lg">
            ✕
          </button>
          ...
          <div className="relative pt-10 px-6 pb-6 z-10 flex flex-col items-center text-center">
            <div className="relative mb-3">
              ...
              <img src={astrologer.profilePic || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} alt={astrologer.fullName} className="relative w-24 h-24 rounded-full object-cover ring-4 ring-white shadow-xl bg-white" />
            </div>
            ...
            <h3 className="text-2xl font-serif font-bold text-[#2B0C39] tracking-wide">
              {astrologer.fullName}
            </h3>
          ...
          <div className="px-6 pb-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 bg-gradient-to-br from-purple-50/60 to-amber-50/40 p-4 rounded-2xl border border-purple-100/80 shadow-inner">
              ...
              <div className="text-center pl-2">
                ...
                <span className="text-lg font-serif font-bold text-amber-600 mt-0.5 block">
                  ★ {astrologer.averageRating || 0}
                  <span className="text-xs font-sans font-normal text-slate-500">
            ...
            <button onClick={onConnect} className="w-full py-3.5 bg-gradient-to-r from-[#2B0C39] via-[#400060] to-[#52007A] hover:opacity-95 text-white font-bold text-xs uppercase tracking-widest rounded-2xl transition-all shadow-xl shadow-purple-950/20 cursor-pointer flex items-center justify-center gap-2">
              <span>
                Connect with {astrologer.fullName?.split(' ')[0]}
              </span>
              <span>→</span>
            </button>
